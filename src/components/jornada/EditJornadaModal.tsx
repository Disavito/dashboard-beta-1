...
const EditJornadaModal: React.FC<EditJornadaModalProps> = ({ isOpen, onClose, jornada, onSuccess }) => {
  ...
  const mutation = useMutation({
    ...
    onMutate: async (updatedData) => {
      ...
      queryClient.setQueryData(['adminJornadas'], (old: any) => {
        if (!old) return old;
        return old.map((j: any) => j.id === jornada.id ? { ...j, ...updatedData } : j);
      });

      // Cerrar modal al instante (Optimistic UI)
      onSuccess();
      return { previousData };
    },
    onSuccess: () => {
    ...
    onError: (error: any, _variables, context) => {
      if (context?.previousData) {
        queryClient.setQueryData(['adminJornadas'], context.previousData);
      }
