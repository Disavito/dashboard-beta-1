...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.List
    ref={ref}
    className={cn(
      "flex overflow-x-auto max-w-full scrollbar-none shrink-0 h-10 items-center justify-start sm:justify-center rounded-lg bg-background p-1 text-textSecondary border border-border shadow-inner",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.Trigger
    ...
    className={cn(
      ...
      "data-[state=active]:bg-surface data-[state=active]:text-primary data-[state=active]:shadow-sm data-[state=active]:font-semibold",
      "hover:text-text",
      className
    )}
    {...props}
