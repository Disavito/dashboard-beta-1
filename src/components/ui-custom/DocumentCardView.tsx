...
const DocumentCardView: React.FC<DocumentCardViewProps> = ({
  ...
  if (data.length === 0) {
    return (
      <div className="text-center py-10 text-textSecondary font-bold border-2 border-dashed border-border rounded-2xl mx-4">
          ...
          <Card key={socio.id} className="w-full bg-card dark:bg-slate-900 border border-border/50 shadow-sm rounded-2xl overflow-hidden">
            ...
            <CardContent className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 py-1">
                <div>
                  <p className="text-[9px] font-black text-muted-foreground/70 uppercase tracking-widest mb-0.5"><MapPin className="w-3 h-3 inline mr-1" />Comunidad</p>
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-foreground/80 uppercase">Mz {socio.mz || '-'} Lt {socio.lote || '-'}</p>
                </div>
              </div>
      ...
      {visibleCount < data.length ? (
        <div ref={observerTarget} className="py-10 flex justify-center w-full">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-[10px] font-black text-muted-foreground/70 uppercase tracking-widest">Pintando más expedientes...</span>
          </div>
        </div>
      ) : (
        <div className="py-8 flex justify-center w-full">
           <span className="text-[10px] font-black text-gray-300 uppercase tracking-widest border border-dashed border-border px-4 py-2 rounded-full">Has llegado al final de la lista</span>
        </div>
      )}
    </div>
  );
