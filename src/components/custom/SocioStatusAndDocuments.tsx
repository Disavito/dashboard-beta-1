...
function SocioStatusAndDocuments({ socioId }: SocioStatusAndDocumentsProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: docsData } = await supabase.from('socio_documentos').select('*').is('deleted_at', null).eq('socio_id', socioId);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      {uploadModal.isOpen && (
        <UploadDocumentModal isOpen onOpenChange={(open) => { if (!open) setUploadModal(prev => ({ ...prev, isOpen: false })); }} socioId={uploadModal.socioId} socioName={uploadModal.socioName} documentType={uploadModal.type} onUploadSuccess={fetchData} />
      )}
    </div>
  );
}
