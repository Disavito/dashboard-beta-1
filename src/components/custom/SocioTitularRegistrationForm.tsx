      ...
      <div className="flex flex-col sm:grid sm:grid-cols-4 sm:items-center gap-2 sm:gap-4">
        ...
        <div className="col-span-full sm:col-span-3 relative">
          <Input
            ...
            className={cn(
              "uppercase rounded-lg border-border bg-background text-foreground focus:ring-primary focus:border-primary transition-all duration-300 h-11 sm:h-9",
    ...
    const formatDateToISO = (dateStr: string | undefined) => {
      ...
      try {
        ...
        if (dateStr.includes('/')) {
          ...
          // It could be YYYY-MM-DD or DD-MM-YYYY
        ...
        if (validYear < 1900 || validYear > new Date().getFullYear()) {
          return ''; // Fecha inverosímil, dejar en blanco
        }
      ...
      const data = response.data.data;
        if (response.data?.success && data) {
          ...
          if (data.date_of_birth) {
            const isoDate = formatDateToISO(data.date_of_birth);
            if (isoDate) setValue('fechaNacimiento', isoDate);
          }
      ...
      try {
        ...
        if (!error && tData) {
          ...
          if (tData.FECHA_NAC && (!form.getValues('fechaNacimiento') || form.getValues('fechaNacimiento') === '')) {
             const isoDate = formatDateToISO(tData.FECHA_NAC);
             if (isoDate) setValue('fechaNacimiento', isoDate);
          }
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      if (socioId !== undefined) {
        ...
        await updateMutation.mutateAsync({ tableName: 'socio_titulares', id: socioId, record: dataToSave });
        onSuccess();
        onClose();
      } else {
        // Optimistic UI Insert via global mutation
        await addMutation.mutateAsync({ tableName: 'socio_titulares', record: dataToSave });
        reset();
            ...
            {activeTab === 'personal' && (
              <div className="space-y-4 animate-in fade-in duration-500">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col sm:grid sm:grid-cols-4 sm:items-center gap-2 sm:gap-4">
                      ...
                      <Popover 
                        open={openLocalitiesPopover} 
                        onOpenChange={(isOpen) => {
                          setOpenLocalitiesPopover(isOpen);
                          if (!isOpen) setLocalitiesSearch('');
                        }}
                      >
                        <PopoverTrigger asChild>
                          <FormControl>
                        ...
                        <PopoverContent className="w-[--radix-popover-trigger-width] p-0 bg-card border-border rounded-xl shadow-lg">
                          <Command>
                            <CommandInput 
                              ...
                              onValueChange={setLocalitiesSearch}
                            />
                            <CommandList>
                              <CommandEmpty>No se encontró localidad.</CommandEmpty>
                              <CommandGroup>
                                {localitiesSearch.trim() && !localitiesSuggestions.some(loc => loc.toLowerCase() === localitiesSearch.trim().toLowerCase()) && (
                                  <CommandItem
                                    forceMount
                                    value={localitiesSearch.trim()}
                                    onSelect={() => {
                                      const val = localitiesSearch.trim();
                                      field.onChange(val);
                                ...
                                {localitiesSuggestions.map((loc) => (
                                  <CommandItem
                                    ...
                                    onSelect={() => {
                                      ...
                                      const addressData = localityAddressMap[loc];
                                      if (addressData) {
                ...
                <div className="flex justify-end mt-6">
                  <Button type="button" onClick={() => setActiveTab('address')} className="bg-secondary text-secondary-foreground h-11 sm:h-9 rounded-xl">
