...
export default function BoletaForm({ /* nextBoletaNumber */ }: BoletaFormProps) { // Removed nextBoletaNumber from destructuring
  ...
  const onSubmit = async (values: BoletaFormValues) => {
    ...
    try {
      ...
      toast.error("Error: " + (error instanceof Error ? error.message : "No se pudo completar la operación."));
    } finally {
      ...
      <CardContent className="p-0">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <Button
              type="submit"
              className="w-full h-16 gap-3 rounded-2xl bg-primary hover:bg-primary/90 text-white text-lg font-black shadow-premium shadow-primary/20 transition-all active:scale-[0.98]"
