...
function SocioDocuments() {
  ...
  const fetchData = useCallback(async (skipLoadingIfCached = false) => {
    if (!skipLoadingIfCached) {
      invalidateCache('socio-documents');
      invalidateCache('partner-documents');
      invalidateCache('people-socios');
    }
    const cached = getCachedData<{ socios: SocioDocumento[]; localidades: string[] }>('socio-documents');
    if (cached.isFresh && cached.data) {
      setSocios(cached.data.socios);
      setLocalidades(cached.data.localidades);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const enriched = (sociosData || []).map(socio => {
        ...
        const hasPlano = socioDocs.some((d: any) => d.tipo_documento === 'Planos de ubicación' && !!d.link_documento);
        const hasMemoria = socioDocs.some((d: any) => d.tipo_documento === 'Memoria descriptiva' && !!d.link_documento);
      ...
      }).filter(Boolean) as SocioDocumento[];

      setCachedData('socio-documents', { socios: enriched, localidades: uniqueLocs });
      setSocios(enriched);
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="p-6 space-y-6 bg-[#FFFFFF] min-h-screen">
                      ...
                      <td className="p-4 text-right">
                        <div className="flex flex-col gap-1 items-end">
                          {socio.is_lote_medido ? (
                            ...
                            <>
                              {!socio.documentos.some((d: any) => d.tipo_documento === 'Planos de ubicación' && !!d.link_documento) && (
                                <Button 
                                  variant="outline" 
                                  size="sm" 
                              ...
                              {!socio.documentos.some((d: any) => d.tipo_documento === 'Memoria descriptiva' && !!d.link_documento) && (
                                <Button 
                                  variant="outline" 
                                  size="sm" 
                                  className="h-7 text-[10px] font-bold border-gray-200 hover:bg-gray-50"
      ...
      {uploadModal.isOpen && (
        <UploadDocumentModal
          isOpen
          onOpenChange={(open) => { if (!open) setUploadModal(prev => ({ ...prev, isOpen: false })); }}
