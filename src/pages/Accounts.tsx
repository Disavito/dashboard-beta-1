...
const Accounts: React.FC = () => {
  ...
  // --- React Query: Fetch all financial data ---
  const { data: financialData, isLoading: loading } = useQuery({
    queryKey: ['supabaseData', 'cuentas', 'ingresos', 'gastos'],
    queryFn: async () => {
      ...
      return {
        accounts: (accRes.data || []) as Cuenta[],
        ingresos: (incRes.data || []) as Ingreso[],
        gastos: (gasRes.data || []) as Gasto[],
      };
    },
    staleTime: 5 * 60 * 1000,
  });
  ...
  const handleTransactionSuccess = () => {
    queryClient.invalidateQueries({ queryKey: ['supabaseData', 'cuentas', 'ingresos', 'gastos'] });
  };
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4">
              ...
              {/* Transferir entre Cuentas button removed - no functionality implemented */}
            </div>
          </div>
        </div>
