...
export default function PresupuestosPage() {
  ...
  const initData = async () => {
    setLoading(true);
    await Promise.all([fetchPresupuestos(), fetchColaboradores()]);
    setLoading(false);
  };
  ...
  const handleClosePresupuesto = async (presupuesto: Presupuesto) => {
    const confirmClose = window.confirm(`¿Estás seguro de cerrar el presupuesto de "${presupuesto.motivo}"? Esto significa que la rendición en campo ha finalizado.`);
              ...
              {isAdminOrFinanzas && (
                <div className="p-4 border-t border-slate-50 bg-slate-50/50 flex gap-2">
                  ...
                  {p.estado === 'Aprobado' && (
                    <Button 
                      size="sm" 
                      onClick={() => handleClosePresupuesto(p)}
